...
<p align="center">
<img src="www/logo-BatNav_officiel.png" width="500" />
</p>
...
:three: Open R terminal **into BatNav folder** 

- If you use windows :scream: Open windows power shell in the BatNav folder (Shift+Right Click on the folder) and launch R terminal with the command bellow. Before change the path according to your R installation.
...
:four: Launch command below in the same R terminal to install the packages required by BatNav.
...
:five: Launch BatNav with this command in the same R terminal:

```r
shiny::runApp("app.r")
```

:warning: If the compiler '*gfortran*' (or '*gcc-fortran*') and the library '*udunits*' aren't installed on your computer the installation of packages (step 4) can fail.